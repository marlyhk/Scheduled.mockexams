...
function renderStartConfirmation() {
  ...
  $("studentShell").innerHTML = `
    <div class="confirm-card card">
      ...
      <button class="btn-block" onclick="startExamNow()">Start Now</button>
...
async function startExamNow() {
  ...
  try {
    const result = await accessRef.transaction((access) => {
    ...
    if (!result.committed) {
      const attemptSnap = await rootRef(`attempts/${currentAccess.id}`).once("value");
      const existing = attemptSnap.val();
      if (existing?.deviceId && existing.deviceId !== deviceId) return studentErrorScreen("This exam has already been opened on another device.");
      if (existing) {
        currentAttempt = { id: currentAccess.id, ...existing };
        startStudentWatchers();
        return existing.status === "active" ? renderActiveExam() : renderFinishedScreen();
      }
      return studentErrorScreen("This one-time password has already been used.");
    }
    const attempt = {
    ...
    await rootRef(`attempts/${currentAccess.id}`).set(attempt);
    currentAttempt = { id: currentAccess.id, ...attempt };
    try {
      await currentUser.updatePassword(randomPassword());
      await accessRef.update({ passwordConsumed: true, passwordConsumedAt: now() });
    } catch (passwordError) {
...
async function addStudentToExam() {
  ...
  await rootRef().update({ [`students/${phone}`]: { name, phone, updatedAt: now(), createdAt: adminData.students?.[phone]?.createdAt || now() }, [`enrollments/${exam.id}/${phone}`]: { ...(enrollmentFor(exam.id, phone) || {}), name, phone, paid: false, allowed: true, updatedAt: now() } });
  showToast("Student added. Mark them Paid to generate access.");
}
...
async function removeStudentFromExam(examId, phone) {
  if (!confirm("Remove this student's access to this exam?")) return;
  const e = enrollmentFor(examId, phone);
  const updates = { [`enrollments/${examId}/${phone}`]: null };
  if (e.activeAccessId) { updates[`access/${e.activeAccessId}/allowed`] = false; updates[`access/${e.activeAccessId}/status`] = "revoked"; }
  await rootRef().update(updates);
  showToast("Student removed from this exam.");
}
